-- ============================================================================
-- squishy — fixture Oracle exhaustive (19c / 19.25)
-- Port du fixture 23ai (test/fixtures/oracle_sample.sql) avec les ajustements
-- imposés par Oracle 19c :
--   * BOOLEAN natif (23c)        → NUMBER(1) + CHECK (v IN (0,1))
--   * JSON natif (21c+)          → CLOB + CHECK (v IS JSON)  (patron canonique 19c)
--   * VECTOR (23ai)              → retiré (pas d'équivalent 19c)
-- Tous les autres types et constructions (IDENTITY 12c, DEFAULT ON NULL 12c,
-- LISTAGG WITHIN GROUP 11gR2, colonnes virtuelles 11g, MV ON DEMAND, triggers
-- compound, pragma AUTONOMOUS_TRANSACTION, collections) sont déjà supportés
-- en 19c sans modification.
--
-- Exécuté par container-registry.oracle.com/database/enterprise:19.3.0.0
-- dans oracle19-sample, après oracle19_init.sql qui a créé l'utilisateur
-- SQUISHY dans la PDB ORCLPDB1.
-- ============================================================================

-- Le setup Oracle EE exécute les scripts comme SYSDBA dans CDB$ROOT ; il faut
-- se reconnecter explicitement en APP_USER dans ORCLPDB1 pour que les
-- CREATE TABLE/TRIGGER landent dans le schéma SQUISHY.
CONNECT squishy/squishy@localhost:1521/ORCLPDB1
...
-- ---------------------------------------------------------------------------
-- t_numeric — NUMBER précision/scale, types numériques binaires,
--             pseudo-BOOLEAN 19c (NUMBER(1) CHECK)
-- ---------------------------------------------------------------------------
CREATE TABLE t_numeric (
  ...
  c_bool             NUMBER(1)                      -- pseudo-BOOLEAN 19c → boolean
                     CONSTRAINT ck_t_numeric_bool CHECK (c_bool IN (0,1))
);
...
-- ---------------------------------------------------------------------------
-- t_json_xml — JSON 19c (CLOB + IS JSON), XMLTYPE
-- VECTOR retiré : pas d'équivalent en 19c.
-- ---------------------------------------------------------------------------
CREATE TABLE t_json_xml (
  id           NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  c_json       CLOB CONSTRAINT ck_t_json_xml_json    CHECK (c_json     IS JSON),
  c_json_chk   CLOB CONSTRAINT ck_t_json_xml_jsonchk CHECK (c_json_chk IS JSON),
  c_xml        XMLTYPE                                -- → xml
);
...
-- (DEFAULT ON NULL est disponible depuis 12c, fonctionne sur 19c.)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- t_generated — colonnes virtuelles calculées (11g+)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- t_identity — IDENTITY BY DEFAULT + UNIQUE inline (12c+)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- orders — FK nommée, check constraint, ENUM émulé via CHECK, metadata JSON 19c
-- ---------------------------------------------------------------------------
CREATE TABLE orders (
  id            NUMBER         GENERATED ALWAYS AS IDENTITY,
  ...
  metadata      CLOB           CONSTRAINT ck_orders_metadata_json CHECK (metadata IS JSON),
  created_at    TIMESTAMP      DEFAULT SYSTIMESTAMP NOT NULL,
...
-- PUBLIC SYNONYM requiert des droits CREATE PUBLIC SYNONYM, retiré pour ce
-- conteneur (squishy n'a pas ce privilège par défaut).
...
-- JSON_VALUE fonctionne en 19c sur colonne CLOB contrainte IS JSON.
...
-- ===========================================================================
BEGIN
  DBMS_OUTPUT.PUT_LINE('squishy oracle 19c fixture loaded');
END;
/
